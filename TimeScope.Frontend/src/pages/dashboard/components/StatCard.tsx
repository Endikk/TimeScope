  ...
  return (
    <Card className={`${colors.bg} border-2 shadow-sm hover:shadow-lg transition-all hover:scale-[1.02] active:scale-[0.98]`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2 px-3 md:px-6 pt-3 md:pt-6">
        <CardTitle className="text-xs sm:text-sm font-medium font-body text-gray-700 truncate pr-2">{title}</CardTitle>
      ...
      <CardContent className="px-3 md:px-6 pb-3 md:pb-6">
        ...
        <p className="text-xs md:text-sm text-gray-600 font-body leading-tight">{description}</p>
        {trend && (
          <div className={`text-xs ${colors.trend} mt-2 font-body font-medium flex items-center gap-1 px-2 py-1 bg-white/50 rounded-md w-fit`}>
